...
| type       | `'mc' \| 'explore' \| 'developer'`                    | `'mc'`   | Change the logo content. Choose between `Marketing Cloud`, `Explore` and `Developer`.                                                               |
| compact    | `boolean`                                             | `false`  | Use the compact version of the Marketing Cloud logo. <u>This attribute has effect only if you set the `type` attribute to `'mc'` (the default)</u>. |
